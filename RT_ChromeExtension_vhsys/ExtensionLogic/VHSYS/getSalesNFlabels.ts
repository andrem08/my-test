...
export default class GetServiceNFLabels {
    ...
    private drawProgressBar(current: number, total: number): void {
        ...
        console.log(`Processing Pages: [${filledBar}${emptyBar}] ${percentage}% (${current}/${total})`);
    }
        ...
        try {
            for (let i = 1; i <= totalPages; i++) {
                ...
                SaleNFLabelUpdate: {
                    progress: i,
                    total: totalPages,
                    label: "SaleNFLabelUpdate"
                }
                })
                this.drawProgressBar(i - 1, totalPages);
                const fetchedData = await this.fetchData(i);
                if (!fetchedData) {
                    console.error(`Failed to fetch data for page ${i}. Skipping this page.`);
                    continue;
                }
                const decoder = new TextDecoder("ISO-8859-1");
                const decodedText = decoder.decode(new TextEncoder().encode(fetchedData));
                const postResult = await this.postData(decodedText);
                if (!postResult) {
                    console.error(`Failed to post data for page ${i}. Skipping this page.`);
                }
                await new Promise(resolve => setTimeout(resolve, 1000));
            }
            this.drawProgressBar(totalPages, totalPages);
            chrome.storage.local.remove("SaleNFLabelUpdate")
